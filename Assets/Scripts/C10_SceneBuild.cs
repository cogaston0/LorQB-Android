...
// C10_SceneBuild.cs (Unity 6, LorQB-Android)
// Unity port of Blender C10_scene_build.py
// THE ONLY CREATOR — creates cubes, ball, seats, hinges, canonical scene
// All other scripts (C12, C13, etc.) manipulate only existing C10 objects
public class C10_SceneBuild : MonoBehaviour
{
    ...
    void BuildScene()
    {
        // Create LorQB_Root parent for flat hierarchy
        GameObject lorqbRoot = new GameObject("LorQB_Root");
        lorqbRoot.transform.position = Vector3.zero;
        lorqbRoot.transform.rotation = Quaternion.identity;
        ...
        CreateHollowCube("Cube_Blue",
            ...
            blueColor, "left", lorqbRoot);

        // Red    bottom-right — 1 hole: top only
        CreateHollowCube("Cube_Red",
            new Vector3(0.51f, 0.5f, -0.51f),
            redColor, null, lorqbRoot);

        // Green  bottom-left  — 1 hole: top only
        CreateHollowCube("Cube_Green",
            new Vector3(-0.51f, 0.5f, -0.51f),
            greenColor, null, lorqbRoot);

        // Yellow top-left    — 2 holes: top + right side
        CreateHollowCube("Cube_Yellow",
            new Vector3(-0.51f, 0.5f, 0.51f),
            yellowColor, "right", lorqbRoot);

        // Create hinges with adjacent cube colors
        CreateHinge("Hinge_Blue_Red", new Vector3(0.51f, 1f, 0f), blueColor, redColor, lorqbRoot);
        CreateHinge("Hinge_Red_Green", new Vector3(0f, 1f, -0.51f), redColor, greenColor, lorqbRoot);
        CreateHinge("Hinge_Green_Yellow", new Vector3(-0.51f, 1f, 0f), greenColor, yellowColor, lorqbRoot);

        CreateBall(new Vector3(0.51f, 0.25f, 0.51f), lorqbRoot);
        CreateSeats(lorqbRoot);

        Debug.Log("LorQB Scene Built with FLAT HIERARCHY — all objects are siblings under LorQB_Root");
    }

    // -------------------------------------------------------------------------
    // REMOVED: EstablishChainHierarchy — replaced with flat hierarchy
    // All cubes and hinges are now siblings under LorQB_Root
    // Movements will use temporary runtime rotation groups
    // -------------------------------------------------------------------------

    // -------------------------------------------------------------------------
    // Hollow cube: 6 rounded faces, holes where required
    // -------------------------------------------------------------------------
    void CreateHollowCube(string cubeName, Vector3 pos, Color color, string sideHole, GameObject parent)
    {
    ...
    void CreateHinge(string name, Vector3 pos, Color colorA, Color colorB, GameObject parent)
    {
    ...
    void CreateBall(Vector3 pos, GameObject parent)
    {
    ...
    // -------------------------------------------------------------------------
    // Create seat empties for each cube
    // -------------------------------------------------------------------------
    void CreateSeats(GameObject parent)
    {
        CreateSeat("Seat_Blue", new Vector3(0.51f, 0.5f, 0.51f), parent);
        CreateSeat("Seat_Red", new Vector3(0.51f, 0.5f, -0.51f), parent);
        CreateSeat("Seat_Green", new Vector3(-0.51f, 0.5f, -0.51f), parent);
        CreateSeat("Seat_Yellow", new Vector3(-0.51f, 0.5f, 0.51f), parent);
    }

    void CreateSeat(string name, Vector3 pos, GameObject parent)
    {
        GameObject seat = new GameObject(name);
        seat.transform.SetParent(parent.transform, false);
